        ...
        stage("CHECKOUT CODE") {
            steps {
                script {
				    ...
				    git url: 'https://github.com/kametepe/ThemeParkCore.git', branch: 'feature/netcore2.2', credentialsId: 'gitlab-default'
                }
            }
        }
...
publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: true, reportDir: "ThemePark.Tests/BuildReports/Coverage", reportFiles: 'index.html', reportName: "CodeCoverage Report"])
       ...
       stage("SAST : QUALITY GATE") {
         ...
         stage("ZIP ARTIFACTS ") {
            steps {
                script {
              zip archive: true, dir: 'ThemePark/bin/Release/netcoreapp2.2/publish/', glob: '', zipFile: "ThemePark-${VERSION}.zip"
                }
